...
def _binary_search(
    ...
    r"""Find the closest passing value to the decision boundary of `predicate`.
    ...
    :param bounds: Optional lower and upper bounds on the input to ``predicate``.
    :param return_sign: If true, also return the direction away from the decision boundary.
    :type return_sign: bool
    :param T: Search type.
    ...
    c_predicate = py_to_c(predicate, c_type=CallbackFnPtr, type_name="bool")
    c_bounds = py_to_c(bounds, c_type=AnyObjectPtr, type_name=RuntimeType(origin='Option', args=[RuntimeType(origin='Tuple', args=[T, T])]))
    c_return_sign = py_to_c(return_sign, c_type=ctypes.c_bool, type_name="bool")
    c_T = py_to_c(T, c_type=ctypes.c_char_p)

    # Call library function.
    lib_function = lib.opendp_internal___binary_search
    lib_function.argtypes = [CallbackFnPtr, AnyObjectPtr, ctypes.c_bool, ctypes.c_char_p]
    lib_function.restype = FfiResult

    output = c_to_py(unwrap(lib_function(c_predicate, c_bounds, c_return_sign, c_T), AnyObjectPtr))
    try:
        output.__opendp_dict__ = {
            '__function__': '_binary_search',
            '__module__': 'internal',
            '__kwargs__': {
                'predicate': predicate, 'bounds': bounds, 'return_sign': return_sign, 'T': T
            },
        }
...
def _exponential_bounds_search(
    predicate,
    T: RuntimeTypeDescriptor
):
    r"""Determine bounds for a binary search via an exponential search.
    ...
    # Call library function.
    lib_function = lib.opendp_internal___exponential_bounds_search
    lib_function.argtypes = [CallbackFnPtr, ctypes.c_char_p]
    lib_function.restype = FfiResult

    output = c_to_py(unwrap(lib_function(c_predicate, c_T), AnyObjectPtr))
    try:
        output.__opendp_dict__ = {
            '__function__': '_exponential_bounds_search',
            '__module__': 'internal',
            '__kwargs__': {
...
def _extrinsic_divergence(
    ...
    output = c_to_py(unwrap(lib_function(c_identifier, c_descriptor), Measure))
    try:
        output.__opendp_dict__ = {
...
def _new_pure_function(
    ...
    [_new_pure_function in Rust documentation.](https://docs.rs/opendp/0.15.1-nightly.20260906.1/opendp/internal/fn._new_pure_function.html)

...
def _extrinsic_measure_descriptor(
    measure: Measure
):
    r"""Retrieve the descriptor value stored in an extrinsic measure.

    .. end-markdown

    :param measure: The ExtrinsicDivergence to extract the descriptor from
    ...
    # Call library function.
    lib_function = lib.opendp_measures___extrinsic_measure_descriptor
    lib_function.argtypes = [Measure]
    lib_function.restype = FfiResult

    output = c_to_py(unwrap(lib_function(c_measure), ExtrinsicObjectPtr))
    try:
        output.__opendp_dict__ = {
            '__function__': '_extrinsic_measure_descriptor',
            '__module__': 'measures',
            '__kwargs__': {
                'measure': measure
            },
...
def approximate(
    ...
    [approximate in Rust documentation.](https://docs.rs/opendp/0.15.1-nightly.20260906.1/opendp/measures/struct.Approximate.html)

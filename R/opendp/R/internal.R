...
`_binary_search` <- function(
  ...
  .T.predicate <- rt_canon("bool")
  .T.bounds <- rt_canon(new_runtime_type(origin = "Option", args = list(new_runtime_type(origin = "Tuple", args = list(.T, .T)))))

  log_ <- new_constructor_log("_binary_search", "internal", new_hashtab(
    list("predicate", "bounds", "return_sign", "T"),
    list(predicate, bounds, unbox2(return_sign), .T)
  ))

  # Assert that arguments are correctly typed.
  rt_assert_is_similar(expected = .T.bounds, inferred = rt_infer(bounds))
  rt_assert_is_similar(expected = "bool", inferred = rt_infer(return_sign))

  # Call wrapper function.
  output <- .Call(
    "internal___binary_search",
    predicate, bounds, return_sign, .T, .T.predicate, .T.bounds,
    log_, PACKAGE = "opendp")
  output
}
...
#' @param predicate A monotonic unary function from a number to a boolean.
#' @param .T Search type.
`_exponential_bounds_search` <- function(
  predicate,
  .T
) {
  # Standardize type arguments.
  ...
  log_ <- new_constructor_log("_exponential_bounds_search", "internal", new_hashtab(
    list("predicate", "T"),
    list(predicate, .T)
  ))
  ...
  # Call wrapper function.
  output <- .Call(
    "internal___exponential_bounds_search",
